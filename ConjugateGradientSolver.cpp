...
double ConjugateGradientSolver::GetResidualNorm(double *x, double *r) const {
    MatrixVectorMultiplication(ia, ja, a, n, x, r); // r = Ax
    LinearCombination(-1, r, 1, b, n); // r = -Ax + b
    
    return sqrt(Dot(r, r, n)); // ||b - Ax||
}
...
int ConjugateGradientSolver::Solve(double *&x, int &iterations, double &res, bool printInfo) {
    ...
    do {
        ...
        if (iterations == 1) {
            LinearCombination(0, p_k, 1, z_k, n); // p_k = z_k
        }
        else {
            LinearCombination(rho_k / rho_prev, p_k, 1, z_k, n); // p_k = z_k + beta * p_k
        }

        MatrixVectorMultiplication(ia, ja, a, n, p_k, q_k); // q_k = A * p_k
        double alpha = rho_k / Dot(p_k, q_k, n); // rho_k / <p_k, q_k>
        LinearCombination(1, x, alpha, p_k, n); // x = x + alpha * p_k
        LinearCombination(1, r, -alpha, q_k, n); // r_k = r_k - alpha * q_k
